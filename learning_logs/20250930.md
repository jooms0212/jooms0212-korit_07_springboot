...
## 정리
- Spring 기반의 애플리케이션 보안(Authentication / Authorization)을 위한 표준 프레임워크. 서블릿 필터(Servlet Filter) 체인을 기반으로 동작하며, 웹 요청에 대한 보안을 체계적으로 처리함. 단순한 로그인 기능 뿐만 아니라, CSRF 공격 방지, 세션 관리, 권한 기반 제어 등 포괄적인 보안 기능을 제공.
## 특징
    ...
    http
      // 1.인가(Authorization) 구성
        .authorizeHttpRequests (authorize -> authorize
          ...
          .anyRequest().perminAll()  // 나머지 경로에 대한 요청들은 접근을 허용함.
        )
...
- 이상의 코드의 상세 설명
  1. authorizeHttpRequests : HTTP 요청에 대한 인가 규칙을 구성.
    ...
    - hasAnyRole(), hasRole() : 특정 역할을 가진 사용자만 접근을 허용. Spring Security에서 역할(Role)은 기본적으로 "ROLE_" 접두사가 붙어서 처리됩니다.(예시 : hasRole("ADMIN")은 "ROLE_ADMIN" 역할을 확인하는 의미입니다)
    - permintAll() : 모든 사용자의접근을 허용.
...
Code Runner -> Java 프로젝트를 실행 시키려면 필요합니다.
Debugger for Java 
...
6. 이상까지의 상황을 반영한 SecurityConfig를 작성.
  - 루트 프로젝트 패키지에 SecuritConfig 클래스 생성.
...
7. 현재 AppUser를 객체 추가하려면 SQL문을 통하거나 혹은 CommandLineRunner를 통해서 해야할 것 같습니다. 이를 통해 생성된 AppUser 객체를 가지고 로그인 성공 시 / 실패 시 를 확인하면 Spring-data-rest 없는 상태의 부분이 구현될 것 같습니다

# 시험 예시 문제
온라인 서점에서 판매할 일반 도서(Book)와 전자책(EBook)을 관리하는 시스템을 구축하고자 합니다. Book 클래스를 상속받는 EBook 클래스를 완성하고, 각 클래스의 객체를 생성하여 정보를 출력하는 프로그램을 작성하세요.
...
2. Animal을 상속하고 makeSound를 오버라이딩 하는 Dog와 Cat 클래스를 만듭니다.
...
6. instanceof 연산자로 Dog 객체를 확인한 후 다운캐스팅하여 fetch 메서드를 호출합니다.
